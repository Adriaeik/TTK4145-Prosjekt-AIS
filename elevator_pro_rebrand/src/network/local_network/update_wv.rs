...
pub fn join_wv(mut my_wv: Vec<u8>, master_wv: Vec<u8>) -> Vec<u8> {
    ...
    let my_self_index = world_view::get_index_to_container(network::SELF_ID.load(Ordering::SeqCst) , my_wv);
    let master_self_index = world_view::get_index_to_container(network::SELF_ID.load(Ordering::SeqCst) , master_wv);
...
pub fn abort_network(wv: &mut Vec<u8>) -> bool {
    ...
    deserialized_wv.elevator_containers.retain(|elevator| elevator.elevator_id == network::SELF_ID.load(Ordering::SeqCst));
    deserialized_wv.set_num_elev(deserialized_wv.elevator_containers.len() as u8);
    deserialized_wv.master_id = network::SELF_ID.load(Ordering::SeqCst);
...
pub fn clear_from_sent_tcp(wv: &mut Vec<u8>, tcp_container: Vec<u8>) -> bool {
    let mut deserialized_wv = serial::deserialize_worldview(&wv);
    let self_idx = world_view::get_index_to_container(network::SELF_ID.load(Ordering::SeqCst) , wv.clone());
